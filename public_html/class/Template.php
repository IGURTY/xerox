...
class Template {
	...
	public function __set($varname, $value){
		if(!$this->exists($varname)) print("var $varname does not exist");
		$stringValue = $value;
		if(is_object($value)){
	...
	private function setBlock($parent, $block) {
		...
		if(1!==preg_match($reg, $str, $m)) print_r("mal-formed block $block");
		$this->setValue($name, '');
	...
	public function block($block, $append = true) {
		...
		if(!in_array($block, $this->blocks)) print_r("block $block does not exist");
